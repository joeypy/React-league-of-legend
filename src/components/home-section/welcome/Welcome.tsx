...
import { bg1, champAhri, champGaren, champAshe, distortion } from '../../../assets/images';

const champImgs = [champAshe, champAhri, champGaren];

const Welcome = (props: any) => {
    useEffect(() => {
        const welcomeImg = document.querySelectorAll('#welcome__img__slide > img');
        let animates: any = [];
        welcomeImg.forEach((item, index) => {
            let nextImg = welcomeImg[index === welcomeImg.length - 1 ? 0 : index + 1].getAttribute('src');
            ...
            <div className="welcome__info relative">
                <div className="welcome__info__content">
                    ...
                    <div className="description mt-4">
                        Team up with friends and test your skills in 5v5 MOBA combat. All the high-skill competition you
                        crave, designed especially for mobile and console with revamped controls and streamlined
                        matches.Explore the living universe of Runeterra through lore, comics, games, and more. Then
                        dive into the community of gamers, cosplayers, musicians, and content creators who are waiting
            ...
            <div className="welcome__img relative">
                <div className="welcome__img__slide" id="welcome__img__slide">
                    {champImgs.map((item, index) => (
                        <img src={item} key={index} alt="champion" />
                    ))}
                </div>
            </div>
        </HomeSection>
